function plot_osc_ARSA()
  ...
  diamDiaph=0.012
    ...
    len_fiber = 3.14 * diamDiaph^2 / 4 / diam_fiber/1000
...
##    y(i) = 1-exp(-(t_WB(i)./l).^k);
  ...
  normRNZ(1:2,1)=150
  time_norm= zeros(2,1)
   time_norm(1,1)=time_new(1)*TimeOrder
   time_norm(2,1)=time_new(size(time_new)(1,1))*TimeOrder
  
  
  plot(time_new*TimeOrder,RNZ/len_fiber,'b',time_new*TimeOrder,sgf3/len_fiber,'r',time_norm,normRNZ,'bk');
  grid minor on
  ylabel ("РНЗ, дБ/км");
  xlabel ( "t, мс");  
  mainPath=pwd
